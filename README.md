...
does not support E2EE: an effective boolean `true` after modeled-wins request
merging is `InvalidArg`, whether it came from a typed field or either raw escape
hatch. Wrong-typed raw values remain server-owned input, and the client never
silently reroutes the request to Chat Completions.
